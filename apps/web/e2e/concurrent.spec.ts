...
test.describe("Concurrent booking — race condition prevention", () => {
  test(
    "TC-CON-01/02 — Only one booking succeeds for last ticket, remaining never goes negative",
    async () => {
      ...
      expect(
        successCount,
        "at most one concurrent booking should succeed"
  ...
  test(
    "TC-CON-03 — Two browser sessions race for last ticket",
    async ({ browser }) => {
      ...
      await Promise.all([
        pageA.goto(`/events/${target.id}`),
        pageB.goto(`/events/${target.id}`),
      ])
      ...
      if (target.remainingTickets === 1) {
        expect(
          aSuccess && bSuccess,
          "both users cannot see 'booked' for the last ticket"
        ).toBe(false)
      }
