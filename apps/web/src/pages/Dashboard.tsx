  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {bookings && bookings.length > 0 && (() => {
        const grouped = bookings.reduce<Record<string, { event: typeof bookings[0]["event"], totalQuantity: number, latestDate: Date }>>((acc, booking) => {
          const key = booking.event.id
          if (!acc[key]) {
            acc[key] = { event: booking.event, totalQuantity: 0, latestDate: booking.createdAt }
          }
          const entry = acc[key]!
          entry.totalQuantity += booking.quantity
          if (booking.createdAt > entry.latestDate) {
            entry.latestDate = booking.createdAt
          }
          return acc
        }, {})
            ...
            {Object.values(grouped).map(({ event, totalQuantity, latestDate }) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">{totalQuantity === 1 ? "ticket" : "tickets"}</p>
